...
import { Alert, BackHandler, Image, ScrollView, StatusBar, StyleSheet, TextInput, ToastAndroid, TouchableOpacity, View } from 'react-native'
...
export default class UserProfile extends React.Component {
    ...
    preferences_hotels = () => {
        return_value = []
        for (let i = 0; i < this.state.preferred_hotels_add_more_count; i++) {
            return_value.push(
    ...
    loyalty_hotels = () => {
        ...
        for (let i = 0; i < this.state.loyalty_hotels_add_more_count; i++) {
            return_value.push(
                <View key={i} style={styles.loyalty_outer_box}>
                    <View style={{ ...styles.each_input_row, marginHorizontal: wp(3), marginTop: hp(1) }}>

                        <View style={{ width: '45%' }}>
                            <Text style={styles.input_label}>Hotel</Text>
                            <RNPickerSelect
                                onValueChange={(value) => {
                                    if (value != 'Select')
                                        this.setState({ frequent_flyer_airline: value })
                                }}
                                placeholder={{}}
                                ...
                                value={this.state.frequent_flyer_airline || 'Select'}
                                style={{ inputAndroid: { ...pickerSelectStyles.inputAndroid, color: this.state.frequent_flyer_airline ? 'black' : 'gray' } }}
                                Icon={() => <View />}
                            />
                        </View>
                        <View style={{ width: '45%' }}>
                            <Text style={styles.input_label}>Number</Text>
                            <TextInput
                                style={styles.input_text}
                            />
                        </View>
    ...
    loyalty_cars = () => {
        ...
        for (let i = 0; i < this.state.loyalty_cars_add_more_count; i++) {
            return_value.push(
                <View key={i} style={styles.loyalty_outer_box}>
                    <View style={{ ...styles.each_input_row, marginHorizontal: wp(3), marginTop: hp(1) }}>

                        <View style={{ width: '45%' }}>
                            <Text style={styles.input_label}>Car</Text>
                            <RNPickerSelect
                                onValueChange={(value) => {
                                    if (value != 'Select')
                                        this.setState({ frequent_flyer_airline: value })
        ...
        axios({
            ...
            .then((response) => {
                ...
                if (response.data.success) {
                    this.setState({ original_user_info: this.state.user_info })
    ...
    update_profile_image = () => {
        ...
        axios({
            method: 'POST',
            url: BookBackendUrl + '/bngupdateprofileimage',
            data: {
                'Authorization': BookAuthentication,
                'referral_code': this.state.partner_referral_code,
                'partner_id': this.state.partner_id,
    ...
    render() {
        ...
        return (
            <ScrollView style={{ backgroundColor: '#F5F5F5', paddingBottom: hp(5) }}>
                ...
                {/* Profile top View */}
                <LinearGradient colors={['#0093DD', '#004A6F']} style={{ borderBottomLeftRadius: wp(6), borderBottomRightRadius: wp(6) }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: hp(4), marginBottom: hp(2), marginHorizontal: wp(4) }}>
                        <TouchableOpacity
                            onPress={() => this.save_data_popup()}
                        >
                            <Text style={{ fontSize: wp(4.5), color: 'white', }}>Back</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: hp(2), marginBottom: hp(6) }}>
                        <TouchableOpacity
                            onPress={() => {
                                ImagePicker.openPicker({
                                    ...
                                    this.setState({ profile_image: image.data, profile_image_mime: image.mime }, this.update_profile_image)
                                })
                            }}
                        >

                            <View style={{ height: wp(20), width: wp(20), borderRadius: wp(20), backgroundColor: 'white' }}>
                                {this.state.profile_image ?
                                    <Image
                                        source={{ uri: `data:${this.state.profile_image_mime};base64,${this.state.profile_image}` }}
                                        style={{ width: wp(20), height: wp(20), borderRadius: wp(10) }}
                                    />
                                    :
                                    <Image
                                        source={{ uri: this.state.user_info.profile_image_url.replace('http://', 'https://') + '&unique=' + this.state.image_unique_value }}
                                        style={{ height: wp(20), width: wp(20), borderRadius: wp(20), resizeMode: 'contain' }}
                                    />
                                }
                                <View style={{ backgroundColor: '#FFBB00', position: 'absolute', top: wp(15), left: wp(12), borderRadius: wp(20), padding: wp(2) }}>
                                    <Image
                                        source={require('../../../assets/images/profile/camera_icon.png')}
                                        style={{ width: wp(5), height: wp(4) }}
                                    />
                                </View>
                            </View>
                ...
                <View style={{ marginVertical: hp(4), marginHorizontal: wp(5) }}>
                    ...
                    <View style={{ marginTop: hp(2.5), width: '50%' }}>
                        <Text style={styles.input_label}>Nationality</Text>
                        {/* <RNPickerSelect
                            onValueChange={(value) => {
                                if (value != 'Select')
                                    this.setState({ user_info: { ...this.state.user_info, nationality: value } })
                            }}
                            placeholder={{}}
                            useNativeAndroidPickerStyle={false}
                            items={this.state.country_data}
                            value={this.state.user_info.nationality || 'Select'}
                            style={{ inputAndroid: { ...pickerSelectStyles.inputAndroid, color: this.state.user_info.nationality ? 'black' : 'gray' } }}
                            Icon={() => <View />}
                        /> */}
                                                    ...
                                                    if (found)
                                                        return_data.push(
                                                            <View key={each_item.id} style={{ flex: 1, flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', width: '100%' }}>
                                                                ...
                                                                {/* <TouchableOpacity
                                                                    style={{ width: '15%', marginLeft: wp(5) }}
                                                                    onPress={() => {

                                                                        let selected_preferred_hotels = this.state.user_info.selected_preferred_hotels
                                                                        selected_preferred_hotels = selected_preferred_hotels.filter((i) => i != selected_item)
