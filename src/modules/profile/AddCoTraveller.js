...
export default class AddCotraveller extends React.Component {
    ...
    send_cotraveller_invite = () => {
        ...
        axios({
            method: 'POST',
            url: BookBackendUrl + '/bngsendcotravellerinvite' + '?data=' + request_data,
            responseType: 'json'
        })
            .then((response) => {
                let data = response.data
                ...
                if (data.success) {
                    this.props.navigation.goBack()
                    ToastAndroid.show('Successfully Invited ' + this.state.user_info.email, ToastAndroid.SHORT)
                }
                else
                    ToastAndroid.show(data.error_info, ToastAndroid.SHORT)
            })
            .catch((error) => {
                console.warn(error)
            })
    ...
    save_data_popup = () => {
        ...
        Alert.alert(
            ...
            [
                { text: 'Discard Changes', onPress: () => this.props.navigation.pop() },
                {
    ...
    render() {
        ...
        return (
            <ScrollView style={{ backgroundColor: '#F5F5F5', paddingBottom: hp(5) }}>
                        ...
                        <View style={{ width: '95%' }}>
                            ...
                            <TextInput
                                ...
                                onEndEditing={() => {
                                    ...
                                    if (!this.state.user_info.email || this.state.user_info.email == '') {
                                    ...
                                    axios({
                                        method: 'POST',
                                        url: BookBackendUrl + '/bnggetnamefromemail' + '?data=' + request_data,
                                        responseType: 'json'
                                    })
                                        .then((response) => {
                                            let data = response.data
                                            ...
                                            if (data.success) {
                                                ...
                                                this.setState({ user_info: { ...this.state.user_info, first_name: data.first_name, last_name: data.last_name } })

                                                Alert.alert(
                                                    'Co-Traveller Found',
                                                    data.first_name + ' ' + data.last_name + '(' + this.state.user_info.email + ')' + ' is already signed up with us. Do you want to invite the him/her to be your cotraveller ?',
                                                    [
                                                        { text: 'No', onPress: () => this.setState({ user_info: { ...this.state.user_info, email: undefined, first_name: '', last_name: '' } }) },
                                                        {
                                                            text: 'Invite', onPress: () => this.send_cotraveller_invite()
                                                        }
                                                    ],
                                                    { cancelable: false },
                                                )
                ...
                <TouchableOpacity
                    style={{ marginTop: hp(2), flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginHorizontal: wp(3) }}
                    onPress={() => this.setState({ invite_user_signup: !this.state.invite_user_signup })}
                >
                    <View
                        style={{ width: wp(4.5), height: wp(4.5), backgroundColor: this.state.invite_user_signup ? '#0093DD' : 'white', borderColor: 'gray', borderWidth: this.state.invite_user_signup ? 1 : 2, borderRadius: 2, marginRight: wp(4) }}
                    />
                    <View>
                        <Text style={{ fontSize: wp(3.5) }}>Invite Co-Traveller to Signup with BookNGoGo</Text>
                    </View>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => {
                        ...
                        if (!this.state.invite_user_signup) {
                            this.update_data()
                            return
                        }

                        this.send_cotraveller_invite()

                    }}
                >
                    <Text style={{ width: '80%', borderRadius: wp(6), fontSize: wp(4.5), color: 'white', height: hp(5), backgroundColor: '#0093DD', textAlign: 'center', textAlignVertical: 'center' }}>Add Co-Traveller</Text>
                </TouchableOpacity>
