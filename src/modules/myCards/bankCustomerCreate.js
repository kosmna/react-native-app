...
class BankCustomerCreate extends React.Component {
    ...
    constructor(props) {
        ...
        this.requesrDwollaCustomer = this.requesrDwollaCustomer.bind(this)
    ...
    requesrDwollaCustomer() {
        ...
        if (!this.state.user_info.ssn || this.state.user_info.ssn == undefined || this.state.user_info.ssn.length == 0) {
            ToastAndroid.show('Please enter SSN', ToastAndroid.SHORT)
            error_text.push('SSN')
        }
        var regexp = /[!@#$%^&*()_+\-=\[\]{};':"\\|<>?]/
        if (regexp.test(this.state.user_info.address_line_1)) {
            ToastAndroid.show('No spatial charecters are allowed', ToastAndroid.SHORT)
            error_text.push('No spatial chracters')
        }
        ...
        if (error_text.length == 0) {
            let payload = {
                ...
                'referral_code': this.props.referral_code,
                'partner_id': this.props.partner_id,
                'firstName': this.state.user_info.first_name,
                'lastName': this.state.user_info.last_name,
                'address1': this.state.user_info.address_line_1,
                'city': this.state.user_info.city,
                'state': this.state.user_info.state_sync_id,
                'dateOfBirth': moment(this.state.user_info.dob).format("YYYY-MM-DD") || ' ',
                'ssn': this.state.user_info.ssn,
                'postalCode': this.state.user_info.zip,
                'email': this.state.user_info.email,

            }
            if (this.state.user_info.address_line_2 != undefined || this.state.user_info.address_line_2 != ' ') {
                payload['address2'] = this.state.user_info.address_line_2 || ' '
            }
            console.log(payload)
            axios({
                method: 'POST',
                url: BackendUrl + "/request/bank_customer?data=" + JSON.stringify(payload),
                responseType: 'json'
            })
                .then((resp) => {
                    this.setState({
                    ...
                    data = res_data
                    if (data.success)
                        ToastAndroid.show('Thank you for submitting your details', ToastAndroid.SHORT)
                    else
                        ToastAndroid.show('Could not able to submit your  details', ToastAndroid.SHORT)
        ...
        return (
            ...
            <ScrollView style={{ flex: 1, height: hp(100), backgroundColor: '#FCFCFC', paddingHorizontal: wp(5), paddingVertical: wp(7) }}>
                ...
                <View style={{ marginHorizontal: wp(5) }}>
                    ...
                    <View style={styles.each_input_row}>
                        <View style={{ width: '45%' }}>
                            <Text style={styles.input_label}>SSN</Text>
                            <TextInput
                                style={styles.input_text}
                                placeholder='SSN'
                                onChangeText={(ssn) => this.setState({ user_info: { ...this.state.user_info, ssn: ssn } })}
                            />
                        </View>
                        <View style={{ width: '45%' }}>
                ...
                <TouchableOpacity
                    ...
                    onPress={this.requesrDwollaCustomer}
                >
...
export default connect(mapStateToProps)(BankCustomerCreate)