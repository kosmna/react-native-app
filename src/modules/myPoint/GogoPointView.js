...
class GogoPointScreen extends React.Component {
    ...
    constructor(props) {
        ...
        delete outlet_data_obj['Bookngogo']

        merchantInfo = {}
        amount_entered = {}
        for (key in outlet_data_obj) {

            merchantInfo[outlet_data_obj[key].id] = {
                enable: false,
                amount: outlet_data_obj[key].total_gogo,
                name: key,
                is_bng: outlet_data_obj[key].is_bng ? true : false
            }

            if (!outlet_data_obj[key].is_bng)
                amount_entered[outlet_data_obj[key].id] = parseInt(outlet_data_obj[key].total_gogo)
        }

        this.state = {
            outlet_data: outlet_data_obj,
    ...
    onToggleSelect = (index) => {
        ...
        if (merchantInfo[index].is_bng) {
            ToastAndroid.show('Cannot convert BookNGoGo points', ToastAndroid.LONG)
            return
        }
        merchantInfo[index].enable = !orgState
        this.setState({ merchantInfo }, () => console.log(this.state))
        for (let i in this.state.merchantInfo) {
            if (this.state.merchantInfo[i].enable)
                visibleConvert = true
        }
        this.setState({ show_convert_button: visibleConvert }, () => console.log(this.state))
    };

    toggleSecurePassword = () => {
        const orgState = this.state.securePassword
        this.setState({ securePassword: !orgState }, () => console.log(this.state))
    }
    ...
    convert_api_call = () => {
        ...
        for (let key in this.state.amount_entered) {
            if (this.state.merchantInfo[key].is_bng)
                continue
                        ...
                        <View style={{ flexDirection: "row", justifyContent: 'center', alignItems: 'center' }}>
                            ...
                            <View style={{ justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: 'gray', borderRadius: wp(1) }}>
                                <TextInput
                                    ...
                                    value={outlet_data[key].is_bng ? '0' : this.state.amount_entered[outlet_data[key].id].toString()}
                                    style={{ fontSize: wp(4), marginHorizontal: wp(2), textAlign: 'center', color: 'black', paddingVertical: 0, marginVertical: 0, height: hp(3.5) }}
                                    autoCorrect={false}
                                    defaultValue={outlet_data[key].is_bng ? 0 : this.state.merchantInfo[outlet_data[key].id].amount.toString()}
                                    keyboardType={'number-pad'}
                                    editable={outlet_data[key].is_bng ? false : true}
                                />
                            </View>
                        </View>
                        <TouchableOpacity
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                ...
                {this.state.show_convert_button &&
                    <TouchableOpacity
                        style={{ justifyContent: 'center', alignItems: 'center', marginVertical: hp(2) }}
                        onPress={() => this.setState({ visibleConvert: true })}
                    >
                        <Text style={{ width: '60%', borderRadius: wp(6), fontSize: wp(4.5), color: 'white', height: hp(5), backgroundColor: '#0093DD', textAlign: 'center', textAlignVertical: 'center' }}>Convert Points</Text>
                    </TouchableOpacity>
                }
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        <View style={{ flex: 0.5, justifyContent: 'space-evenly', backgroundColor: '#fff' }}>
                            ...
                            <View style={{ flexDirection: "row", justifyContent: 'flex-start', alignItems: 'center', marginHorizontal: wp(5) }}>
                                <Text style={{ flex: 0.4, fontSize: wp(4), color: '#918888', }}>Convert your</Text>
                                <Image
                                    style={{ height: wp(9.5), width: wp(20), resizeMode: 'contain', marginTop: hp(0.5), marginRight: wp(5) }}
                                    source={require('../../../assets/images/rewards/gogo$.png')}
                                />
                            ...
                            {/* Buttons */}
                            <View style={{ flexDirection: "row", justifyContent: 'space-around', marginHorizontal: wp(5) }}>
...
export default connect(mapStateToProps)(GogoPointScreen)